...
#     """Helper to return JSON or text safely for display."""
#     try:
#         return resp.json()
#     except Exception:
#         return resp.text
...
#                 # streamlit file-like -> bytes
#                 files.append(("zip_files", (f.name, f.getvalue(), "application/zip")))
#             for f in uploaded_xmls:
#                 files.append(("xml_files", (f.name, f.getvalue(), "application/xml")))

#             resp = requests.post(f"{FASTAPI_URL}/task-creator/upload-assets", files=files)
#             if resp.status_code == 200:
...
#             # Send empty string if left blank (not JSON null)
...
#     # ---------------------------
#     # 3️⃣ Detect Overlaps & Single-Annotator Tasks
#     # ---------------------------
...
#         if len(parts) >= 2:
#             clip_name = '_'.join(parts[1:-1])  # base clip name
...
#     # ---------------------------
#     # 5️⃣ Approve Single-Annotator Tasks
#     # ---------------------------
...
#     frames_root_dir = st.text_input("Frames root directory (optional)", value="")

#     if st.button("🚀 Generate Dataset"):
#         payload = {"output_filename": output_filename}
#         if frames_root_dir.strip():
#             payload["frames_root_directory"] = frames_root_dir.strip()
...
#             st.download_button("⬇️ Download Dataset", data=resp.content, file_name=output_filename)
#         else:
#             st.error(f"❌ Error: {safe_json_or_text(resp)}")
