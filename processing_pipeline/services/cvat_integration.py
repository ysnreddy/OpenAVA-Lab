...
class CVATClient:
    ...
    # -------- AUTH (Token-based, no CSRF) --------
    def login(self) -> bool:
        """Log in to CVAT and store API token. No CSRF required on modern CVAT."""
        try:
            url = f"{self.host}/api/auth/login"
            ...
            key = data.get("key")
            if not key:
                logger.error(f"Login response did not contain token: {data}")
                return False

            self.token = key
            # All subsequent requests will carry the token
    ...
    # -------- PROJECTS --------
    def create_project(self, name: str, labels: List[Dict[str, Any]]) -> int | None:
        try:
    ...
    def create_tasks_from_assignments(
        ...
        for annotator, clips in assignments.items():
            ...
            for i, clip_name in enumerate(clips):
                ...
                # Give the backend a small breather before importing annotations
                time.sleep(2)
                ...
                if self.assign_user_to_task(task_id, annotator):
                    logger.info(f"✓ Task {task_id} assigned to {annotator}")
                    created_tasks.append(
                        {"task_id": task_id, "annotator": annotator, "clip": clip_name}
                    )
                else:
                    logger.error(f"✗ Failed to assign task {task_id} to {annotator}")
    ...
    def create_task(self, name: str, project_id: int, labels: List[Dict[str, Any]] | None = None) -> int | None:
        try:
            logger.info(f"Creating task '{name}' in project {project_id}...")
    ...
    def upload_data_to_task(self, task_id: int, zip_file_path: str) -> bool:
        try:
            ...
            with open(zip_file_path, 'rb') as fh:
                files = {
                    # CVAT accepts multiple files as client_files[]; index or [] both work
                    'client_files[0]': (os.path.basename(zip_file_path), fh, 'application/zip')
                }
                data = {
                    'image_quality': '95',
                    'use_zip_chunks': 'true',
    ...
    def import_annotations(self, task_id: int, xml_file: str) -> bool:
        try:
            url = f"{self.host}/api/tasks/{task_id}/annotations?action=import&format=CVAT%201.1"
            ...
            if resp.status_code not in (202, 201, 200):
                logger.error(f"Annotation import failed to start: {resp.status_code} - {resp.text}")
                return False

            rq_id = resp.json().get("rq_id")
            if not rq_id:
                logger.error(f"No rq_id returned in response: {resp.text}")
                return False

            logger.info(f"Started annotation import job {rq_id} for task {task_id}")

            # Poll the request status until finished
            while True:
                status_resp = self._make_authenticated_request("GET", f"{self.host}/api/requests/{rq_id}")
                if status_resp.status_code != 200:
                    logger.error(f"Failed to check job status: {status_resp.status_code} - {status_resp.text}")
                    return False

                status = status_resp.json().get("status")
                if status == "finished":
                    logger.info(f"✓ Annotation import completed for task {task_id}")
                    return True
                elif status == "failed":
                    logger.error(f"✗ Annotation import failed for task {task_id}: {status_resp.json()}")
                    return False
                else:
                    time.sleep(2)
    ...
    def assign_user_to_task(self, task_id: int, username: str) -> bool:
        try:
            ...
            if resp.status_code != 200:
                logger.error(f"Failed to search for user: {resp.status_code} - {resp.text}")
                return False
            ...
            # find jobs for the task
            resp = self._make_authenticated_request('GET', f"{self.host}/api/jobs", params={"task_id": task_id})
            if resp.status_code != 200:
                logger.error(f"Failed to get jobs for task: {resp.status_code} - {resp.text}")
                return False
            ...
            for job in jobs:
                j_id = job['id']
                j_resp = self._make_authenticated_request(
                    'PATCH',
                    f"{self.host}/api/jobs/{j_id}",
                    json={'assignee': user_id}
                )
                if j_resp.status_code == 200:
                    logger.info(f"✓ Assigned '{username}' to job {j_id}")
                else:
                    logger.error(f"Failed to assign job {j_id}: {j_resp.status_code} - {j_resp.text}")
                    ok = False
            return ok
    ...
    def wait_for_data_upload_completion(self, task_id: int, max_wait: int = 600) -> bool:
        """Poll task status until data import finishes or fails."""
        start = time.time()
        logger.info(f"Waiting for data upload completion for task {task_id}...")
        ...
        while time.time() - start < max_wait:
            ...
            if resp.status_code == 200:
                ...
                if state in ('finished', 'completed'):
                    ...
                    return True
                if state in ('failed',):
                    logger.error(f"✗ Data upload failed for task {task_id}: {resp.text}")
                    return False
                # queued/started/processing
                time.sleep(3)
                continue
            else:
                logger.warning(f"Status check returned {resp.status_code}; retrying in 3s")
                time.sleep(3)

        logger.error(f"Timeout waiting for data upload completion for task {task_id}")
        return False

    def wait_for_annotation_import_completion(self, task_id: int, max_wait: int = 300) -> bool:
        """Poll annotation count until it increases (simple, robust check)."""
        try:
            start = time.time()
            logger.info(f"Waiting for annotation import completion for task {task_id}...")
            initial = self.get_annotation_count(task_id)

            while time.time() - start < max_wait:
                current = self.get_annotation_count(task_id)
                if current > initial:
                    logger.info(f"✓ Annotations imported for task {task_id} (count: {current})")
                    return True
                time.sleep(3)
    ...
    def get_annotation_count(self, task_id: int) -> int:
        try:
            resp = self._make_authenticated_request('GET', f"{self.host}/api/tasks/{task_id}/annotations")
            if resp.status_code == 200:
                data = resp.json()
                return len(data.get('tracks', [])) + len(data.get('shapes', []))
            return 0
        except Exception as e:
            logger.error(f"Exception getting annotation count: {e}")
            return 0
